...
import { TrendingUp, MapPin, Clock, RefreshCw, ChevronDown, ChevronUp, Bell, DollarSign, BarChart3 } from 'lucide-react';
...
const CoconutRateApp = () => {
  ...
  const fetchPriceHistory = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/history?days=7`);
      const data = await response.json();
      
      if (data.success && data.data.chart_data) {
        setPriceHistory(data.data.chart_data);
      }
    } catch (error) {
  ...
  const fetchDistrictPrices = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/districts`);
      const data = await response.json();
      
      if (data.success && data.data.districts) {
        setDistrictPrices(data.data.districts);
      }
    } catch (error) {
  ...
  const handleRefreshPrice = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/price/refresh`, {
        method: 'POST',
        headers: {
  ...
  // Handle price confirmation (YES/NO)
  const handlePriceConfirmation = async (isCorrect) => {
    try {
      const response = await fetch(`${API_BASE_URL}/verify`, {
        ...
        body: JSON.stringify({
          ...
          market: userLocation,
          price: isCorrect ? null : userSubmittedPrice
        }),
      });
  ...
  // Submit new price
  const handlePriceSubmission = async (e) => {
    e.preventDefault();
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/submit`, {
        ...
        body: JSON.stringify({
          price: userSubmittedPrice,
          district: userDistrict,
          market: userLocation,
          notes: 'User submitted price'
        }),
      });
  ...
  // Handle district price verification
  const handleDistrictPriceVerification = async (district, isCorrect) => {
    if (isCorrect) {
      try {
        const response = await fetch(`${API_BASE_URL}/verify`, {
          method: 'POST',
  ...
  // Submit district price correction
  const handleDistrictPriceSubmission = async (e) => {
    e.preventDefault();
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/submit`, {
        ...
        body: JSON.stringify({
          price: districtSubmittedPrice,
          district: districtPriceSubmission,
          market: districtSubmittedLocation,
          notes: 'District price correction'
        }),
      });
      ...
      console.error('Error submitting district price:', error);
      alert('Failed to submit price. Please try again.');
    }
  };
  ...
  // Mock market data (this would ideally come from backend)
  const marketDetails = [
    { 
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-emerald-50">
      {/* Header - Fixed */}
      <div className="fixed top-0 left-0 right-0 bg-gradient-to-r from-green-600 to-emerald-600 text-white p-6 shadow-lg z-50">
      ...
      <div className="pt-24">
        <div className="max-w-4xl mx-auto p-4 pb-32">
          ...
          {/* TAB 1: PRICE - Only Main Price Card */}
          {activeTab === 'price' && (
            <div className="mt-4">
              ...
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl shadow-lg p-6">
                ...
                <p className="text-sm text-gray-600 mb-2">
                  This is the average price calculated from 5+ trusted agricultural market sources:
                </p>
          ...
          {/* TAB 2: DISTRICT - Only District-wise Prices */}
          {activeTab === 'district' && (
            <div className="mt-4">
              ...
              {/* About District Prices - Info Section */}
              <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-2xl shadow-lg p-6">
                <h3 className="font-bold text-gray-800 mb-3 flex items-center gap-2">
                  <MapPin className="w-5 h-5 text-green-600" />
                  About District Prices
                </h3>
                <p className="text-sm text-gray-600 mb-2">
                  District prices are calculated based on:
                </p>
                <ul className="text-xs text-gray-500 space-y-1">
                  <li>• Real-time data from local mandis and markets</li>
                  <li>• Verified submissions from farmers and traders</li>
                  <li>• Historical price trends for each district</li>
                  <li>• Seasonal demand and supply variations</li>
                  <li>• Quality variations (size, maturity, dehusking)</li>
                </ul>
                <p className="text-xs text-gray-500 mt-3 italic">
                  * Click YES/NO to verify prices and help improve accuracy
                </p>
              </div>
            </div>
          )}

          {/* TAB 3: TRENDS - Only Price History & Trends */}
          {activeTab === 'trends' && (
            <div className="mt-4">
              <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  {stats && (
                    <div className="mt-6 grid grid-cols-3 gap-4 text-center">
                      <div className="bg-green-50 p-4 rounded-lg">
                        ...
                        <p className="text-2xl font-bold text-green-600">₹{stats.seven_day_average}</p>
                      </div>
                      <div className="bg-blue-50 p-4 rounded-lg">
                        <p className="text-sm text-gray-600">Weekly Change</p>
                        <p className="text-2xl font-bold text-blue-600">{stats.weekly_change}</p>
                      </div>
                      <div className="bg-purple-50 p-4 rounded-lg">
                        <p className="text-sm text-gray-600">Today's High</p>
                        <p className="text-2xl font-bold text-purple-600">₹{stats.max_today}</p>
                      </div>
                    </div>
                  )}
                </div>
              ...
              {/* About Price Trends - Info Section */}
              <div className="bg-gradient-to-r from-orange-50 to-yellow-50 rounded-2xl shadow-lg p-6">
                <h3 className="font-bold text-gray-800 mb-3 flex items-center gap-2">
                  <TrendingUp className="w-5 h-5 text-orange-600" />
                   Market Predictions
                </h3>
                <p className="text-sm text-gray-600 mb-2">
                  Trend analysis helps predict future price movements:
                </p>
                <ul className="text-xs text-gray-500 space-y-1">
                  <li>• Weekly trends show short-term market direction</li>
                  <li>• Seasonal patterns affect coconut prices annually</li>
                  <li>• Monsoon impacts harvest and supply chain</li>
                  <li>• Festival seasons increase demand significantly</li>
                  <li>• Export demand influences domestic prices</li>
                </ul>
                <p className="text-xs text-gray-500 mt-3 italic">
                  * Data is analyzed daily to provide accurate trend predictions
                </p>
              </div>
            </div>
          )}

          {/* TAB 4: MARKETS - Only Local Markets */}
          {activeTab === 'markets' && (
            <div className="mt-4">
              <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
              ...
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl shadow-lg p-6">
                ...
                <ul className="text-xs text-gray-500 space-y-1">
                  <li>• Real-time price updates from each market</li>
                  <li>• Trading volume indicators (High/Medium/Low)</li>
                  <li>• Current market status (Open/Closed)</li>
                  <li>• Operating hours for optimal visiting times</li>
                  <li>• Contact details for market inquiries</li>
                  <li>• Exact locations with address details</li>
                </ul>
                <p className="text-xs text-gray-500 mt-3 italic">
                  * Market data is verified daily for accuracy and timeliness
                </p>
              </div>
            </div>
          )}

          {/* TAB 5: ALERTS - Only Price Alerts */}
          {activeTab === 'alerts' && (
            <div className="mt-4">
              {/* Alert Settings Card */}
              <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
              ...
              <div className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-2xl shadow-lg p-6">
                ...
                <ul className="text-xs text-gray-500 space-y-1">
                  <li>• <strong>Price Threshold:</strong> Get notified when price reaches your target</li>
                  <li>• <strong>Price Increase:</strong> Alert when daily price goes up significantly</li>
                  <li>• <strong>Price Drop:</strong> Notification for price decreases in your district</li>
                  <li>• <strong>Market Updates:</strong> Updates on new data from local markets</li>
                  <li>• <strong>Trend Alerts:</strong> Notifications about emerging price trends</li>
                </ul>
                <p className="text-xs text-gray-500 mt-3 italic">
                  * Alerts help you make informed buying/selling decisions
                </p>
              </div>
            </div>
          )}
        ...
        <div className="max-w-4xl mx-auto px-4 py-3 flex justify-around">
          <button 
            ...
            className={`flex flex-col items-center gap-1 ${
              activeTab === 'price' ? 'text-green-600' : 'text-gray-400'
            }`}
          >
          ...
          <button 
            ...
            className={`flex flex-col items-center gap-1 ${
              activeTab === 'district' ? 'text-green-600' : 'text-gray-400'
            }`}
          >
            <div className="w-8 h-8 flex items-center justify-center">
          ...
          <button 
            ...
            className={`flex flex-col items-center gap-1 ${
              activeTab === 'trends' ? 'text-green-600' : 'text-gray-400'
            }`}
          >
            <div className="w-8 h-8 flex items-center justify-center">
